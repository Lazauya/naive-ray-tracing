...
namespace objects
{
	class object
	{
		...
		rgbColor specular_;
		F64 shininess_;
		rgbColor reflectance_; //the color to reflect
		F64 scattering_; //the angle of light scattering; the roughness of the object surface
		F64 transparency_; //the transparency of the object
		F64 eta_; //the refraction constant
	public:
		object(rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0) : color_(c), shininess_(sh), specular_(s),
	       												transparency_(t), scattering_(scat), reflectance_(ref), eta_(eta) {};
		...
		inline F64 transparency() const { return transparency_; };
	...
	protected:
		...
		plane(rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0) :
		       	object(c, sh, s, ref, t, scat, eta),
		       	normal_(vec3d()),
		       	includedPoint_(point()) {};
	public:
		//initialize plane with given parameters (coefficients)
		plane(F64 c1, F64 c2, F64 c3, F64 c4, rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0); 
		plane(vec3d n, point p, rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0) :
		       	object(c, sh, s, ref, t, scat, eta),
		       	normal_(n),
		       	includedPoint_(p) {};
	...
	public:
		triangle(point p1, point p2, point p3, rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0);
	...
	public:
		rectangle(point p1, point p2, vec3d norm, rgbColor c, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0);
	...
	class sphere : public object
	{
		...
		F64 radius;
	public:
		sphere(point cen, F64 rad, rgbColor col, F64 sh, rgbColor s, rgbColor ref=rgbColor(0,0,0,true), F64 t=0.0, F64 scat=0.0, F64 eta=1.0) :
		       	object(col, sh, s, ref, t, scat, eta),
		       	center(cen),
		       	radius(rad) {};
